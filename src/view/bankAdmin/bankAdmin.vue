...
import { getPropject, getPageDepts, workerSalaryGetPageList, payoff, getTime, payOff } from '@/api/bankAdmin/bankAdmin'
          ...
          render: (h, params) => {
            return h('div', [
              h('Button', {
                ...
                on: {
                  click: () => {
                    this.dataList[params.index].loading = true
    ...
    getList () {
      ...
      workerSalaryGetPageList(this.pageNum, this.selectValue, this.pidValue, this.timePbId).then(res => {
        ...
        res.info.data.map((res) => {
          ...
          console.log(this.dataList, 'sssssssss')
          // disabled: params.row.state
          let state = true
